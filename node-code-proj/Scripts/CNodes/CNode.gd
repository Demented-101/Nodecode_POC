...
enum pinTypes {Exc, DataAny, Bool, Int, _String, Direction}
...
func populate_pins() -> void:
	...
	for i:int in program.output_count:
		...
		new_out_pin.setup(outputs.values()[i], self, i, outputs.keys()[i], false)
	...
	for i:int in program.input_count:
		...
		new_in_pin.setup(inputs.values()[i], self, i, inputs.keys()[i], false)
